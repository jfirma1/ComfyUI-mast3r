...
@torch.no_grad()
def clean_pointcloud(confs, intrinsics, cams, depthmaps, pts3d, tol=0.001, max_bad_conf=0):
    """Standalone function to clean pointcloud by checking depth consistency.
    
    For each 3D point, check if it's occluded when viewed from other cameras.
    If a point is in front of another point with higher confidence, reduce its confidence.
    
    Args:
        confs: list of confidence maps
        intrinsics: list of camera intrinsics (K matrices)
        cams: list of world-to-camera transforms
        depthmaps: list of depth maps
        pts3d: list of 3D point clouds
        tol: tolerance for depth comparison
        max_bad_conf: maximum confidence for bad points
    ...
    n_imgs = len(confs)
    res_confs = deepcopy(confs)
    
    for i in range(n_imgs):
        pts3d_i = pts3d[i]
        if isinstance(pts3d_i, torch.Tensor):
            Hi, Wi = pts3d_i.shape[:2] if pts3d_i.ndim >= 2 else (int(np.sqrt(len(pts3d_i))), int(np.sqrt(len(pts3d_i))))
        else:
            Hi, Wi = pts3d_i.shape[:2] if pts3d_i.ndim >= 2 else (int(np.sqrt(len(pts3d_i))), int(np.sqrt(len(pts3d_i))))
        ...
        for j in range(n_imgs):
            ...
            try:
                # Get shapes
                conf_j = confs[j]
                if isinstance(conf_j, torch.Tensor):
                    Hj, Wj = conf_j.shape[:2]
                else:
                    Hj, Wj = conf_j.shape[:2]
                
                # Project 3d points from view i into view j
                pts_flat = pts3d_i.reshape(-1, 3) if hasattr(pts3d_i, 'reshape') else torch.tensor(pts3d_i).reshape(-1, 3)
                if not isinstance(pts_flat, torch.Tensor):
                    pts_flat = torch.tensor(pts_flat)
                
                cam_j = cams[j] if isinstance(cams[j], torch.Tensor) else torch.tensor(cams[j])
                K_j = intrinsics[j] if isinstance(intrinsics[j], torch.Tensor) else torch.tensor(intrinsics[j])
                
                # Transform points to camera j coordinates
                proj = geotrf(cam_j.to(pts_flat.device), pts_flat.to(cam_j.device))
                proj = proj.reshape(Hi, Wi, 3)
                proj_depth = proj[:, :, 2]
                
                # Project to image coordinates
                uv = geotrf(K_j.to(proj.device), proj, norm=1, ncol=2)
                u, v = uv[..., 0].round().long(), uv[..., 1].round().long()
                
                # Check which points are in the visible cone
                msk_i = (proj_depth > 0) & (0 <= u) & (u < Wj) & (0 <= v) & (v < Hj)
                ...
                # Get valid indices
                v_valid, u_valid = v[msk_i], u[msk_i]
                
                # Get depth values from depthmap j at projected locations
                depthmap_j = depthmaps[j] if isinstance(depthmaps[j], torch.Tensor) else torch.tensor(depthmaps[j])
                depthmap_j = depthmap_j.to(proj_depth.device)
                
                conf_i = res_confs[i] if isinstance(res_confs[i], torch.Tensor) else torch.tensor(res_confs[i])
                conf_j_t = conf_j if isinstance(conf_j, torch.Tensor) else torch.tensor(conf_j)
                conf_i = conf_i.to(proj_depth.device)
                conf_j_t = conf_j_t.to(proj_depth.device)
                
                # Find bad points (in front but less confident)
                ref_depth = depthmap_j[v_valid, u_valid]
                cur_depth = proj_depth[msk_i]
                cur_conf = conf_i.reshape(Hi, Wi)[msk_i] if conf_i.numel() == Hi * Wi else conf_i[msk_i]
                ref_conf = conf_j_t[v_valid, u_valid]
                
                bad_points = (cur_depth < (1 - tol) * ref_depth) & (cur_conf < ref_conf)
                
                # Update confidence for bad points
                if bad_points.any():
                    bad_msk_i = msk_i.clone()
                    bad_msk_i[msk_i] = bad_points
                    
                    if isinstance(res_confs[i], torch.Tensor):
                        res_confs[i] = res_confs[i].reshape(Hi, Wi)
                        res_confs[i][bad_msk_i] = res_confs[i][bad_msk_i].clip(max=max_bad_conf)
                    else:
                        res_confs[i] = np.array(res_confs[i]).reshape(Hi, Wi)
                        res_confs[i][bad_msk_i.cpu().numpy()] = np.clip(res_confs[i][bad_msk_i.cpu().numpy()], a_min=None, a_max=max_bad_conf)
                        
            except Exception as e:
                # If something goes wrong, just skip this pair
                continue
