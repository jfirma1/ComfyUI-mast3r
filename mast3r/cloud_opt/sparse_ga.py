...
def sparse_scene_optimizer(imgs, subsample, imsizes, pps, base_focals, core_depth, anchors, corres, corres2d,
    ...
    def loss_dust3r(cam2w, pts3d, pix_loss):
        ...
        if cf_sum != 0.:
            return loss / cf_sum
        # Return a zero tensor with gradient connection if no valid slices
        return torch.tensor(0., device=device, dtype=dtype, requires_grad=True) * sum(p.sum() for p in pts3d)
    ...
    def loss_3d(K, w2cam, pts3d, pix_loss):
        ...
        # Return a zero tensor with gradient connection if no valid slices
        # This creates a dummy loss that still allows gradients to flow
        dummy_loss = torch.tensor(0., device=device, dtype=dtype)
        for p in pts3d:
            dummy_loss = dummy_loss + 0. * p.sum()
        return dummy_loss
    ...
    def optimize_loop(loss_func, lr_base, niter, pix_loss, lr_end=0):
        ...
        if not trainable_params:
            print("WARNING: No trainable parameters! Forcing gradients on quats, trans, log_sizes")
            for q in quats:
            ...
            trainable_params = [p for p in (quats + trans + log_sizes) if p.requires_grad]
        
        print(f"Trainable params: {len(trainable_params)}")
        optimizer = torch.optim.Adam(trainable_params, lr=1, weight_decay=0, betas=(0.9, 0.9))
    ...
    # Ensure at least some parameters require gradients
    has_trainable = any(q.requires_grad for q in quats) or any(t.requires_grad for t in trans) or any(s.requires_grad for s in log_sizes)
    if not has_trainable:
        print("Warning: No trainable parameters found, enabling gradients for all poses")
        for i in range(len(imgs)):
